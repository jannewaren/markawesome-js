...
- **Web Awesome manifest coverage test** (`test/webawesome-manifest-coverage.test.ts`) — a byte-for-byte mirror of the Ruby spec: it checks every enum the transformers accept against a vendored slice of WA's Custom Elements Manifest (pinned to **WA 3.10.0**) for **DRIFT** and **GAP** across the same 39 `(constant → wa-tag/attribute)` mappings across 15 components, surfacing the 6 non-inline-enum entries (`wa-icon` family/variant/animation/canvas, `wa-video` controls/preload) as `it.skip`. The module-private `*_ATTRIBUTES` consts (plus `callout` `VARIANTS`, `date` `RELATIVE_FORMATS`/`RELATIVE_NUMERICS`/`GRANULAR_ENUMS`, `video` `CONTROLS_VALUES`/`PRELOAD_VALUES`) gain an `export` so the test can read the **live** values — additive only; `index.ts` and the tsup bundle entry are untouched. The fixture `test/fixtures/webawesome-enums.json` is a **derived artifact** regenerated with `npm run update-wa-manifest <version>` (`scripts/update-wa-manifest.mjs`) — never hand-edited, and byte-identical to `markawesome`'s copy. Not published (`test/` and `scripts/` are outside `package.json` `files`); no version bump. See CLAUDE.md → "Web Awesome manifest coverage" for the WA-bump workflow.
